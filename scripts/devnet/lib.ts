...
export const keypairPath = resolve(process.env.DEVNET_KEYPAIR_PATH ?? '.devnet/keypair.json')
export const solanaNetwork = getSolanaNetwork()
export const rpcUrl = getRpcUrl()
export const airdropRpcUrl = process.env.SOLANA_AIRDROP_RPC_URL ?? 'https://api.devnet.solana.com'
...
export async function writeKeypair(keypair: Keypair) {
  ...
  await writeFile(keypairPath, JSON.stringify(Array.from(keypair.secretKey)), { mode: 0o600 })
}

export async function loadKeypair() {
  if (!existsSync(keypairPath)) {
    throw new Error(`Missing devnet keypair at ${keypairPath}. Run pnpm devnet:keypair first.`)
  }
...
export async function writeSignedTransaction(name: string, serializedBase64: string) {
  const outputPath = resolve(process.env.DEVNET_SIGNED_TX_PATH ?? `.devnet/${name}.base64`)
