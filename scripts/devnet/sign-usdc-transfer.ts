...
import { getConnection, loadKeypair, requireArg, writeSignedTransaction } from './lib.js'

const keypair = await loadKeypair()
const recipient = new PublicKey(requireArg(process.argv[2], 'recipient public key'))
const amount = BigInt(process.argv[3] ?? '1')
const mint = new PublicKey(requireArg(process.env.DEVNET_USDC_MINT, 'DEVNET_USDC_MINT'))
...
if (!destinationInfo) {
  instructions.push(createAssociatedTokenAccountInstruction(keypair.publicKey, destinationAta, recipient, mint))
}

instructions.push(createTransferCheckedInstruction(sourceAta, mint, destinationAta, keypair.publicKey, amount, decimals))
...
const outputPath = await writeSignedTransaction('signed-usdc-transfer', serializedBase64)
